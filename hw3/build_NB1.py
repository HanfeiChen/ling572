...
class NaiveBayes:
    ...
    def train(self, x: List[Dict[str, int]], y: List[str]) -> None:
        ...
        for xi, yi in zip(x, y):
            ...
            for feat_name, count in xi.items():
                ...
                if count >= 1:
                    self.feat_class_counter[(feat_name, yi)] += 1
    ...
    def cond_prob(self, feat_name: str, class_name: str) -> float:
        return (self.feat_class_counter[(feat_name, class_name)] + self.cond_prob_delta) \
                / (self.class_counter[class_name] + self.cond_prob_delta * 2)
    ...
    def predict_probability(self, xi: Dict[str, int]) -> Dict[str, float]:
        ...
        for c in self.class_set:
            ...
            for f, count in xi.items():
                if count >= 1:
                    cond_prob = self.cond_prob(f, c)
                    # print(cond_prob)
                    log_prob += math.log10(cond_prob / (1. - cond_prob))
            log_probs[c] = log_prob
        #  P(c, xi) = exp( log P(xi | c) + log P(c) )
...
if __name__ == '__main__':
    ...
    nb = NaiveBayes(prior_delta=CLASS_PRIOR_DELTA, cond_prob_delta=COND_PROB_DELTA)
